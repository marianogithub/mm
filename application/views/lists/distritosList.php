...
echo "<thead><tr class='bg-inverse'>";
    ...
    $urlZona = Kohana::$index_file . "/Zona/index/id=";
	//datos de la tabla.-
	foreach( $visor->getListado() as $key => $value ) {
		$id = $value->pk();
        $cantidadZona = $value->getCantidadZonas();
        $zonaBadge = "<span class='badge pull-right'>$cantidadZona</span>";
        $nombreColumnaZona = $cantidadZona == 0 ? "No hay Zonas" : "Ver Zonas $zonaBadge";
		echo "<tr>";
		echo "<td>$value->nombredistrito</td>";
        echo "<td>" . html::anchor( $urlZona . $id, $nombreColumnaZona ) . "</td>";
