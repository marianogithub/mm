...
/**
  * 
  */
class Controller_ExportarInspeccionesBase extends App_Controller_Template {
	...
	function __construct($request, $response) {
		...
		$this->titulo			= "Impresión de Inspecciones";
		$this->encabezado		= "Impresión de Inspecciones";

		$this->nombreColumnas =
            Array(
                "Fecha Solicitada", "Fecha Efect. Insp.", "Gremio", "Tipo Inspeccion", "Nivel", "Cartel de Obra","Sector", "Comentarios", "Total/Parcial", "Estado","Inspector"
            );
		$this->nombreTabla		= "inspecciones";
		$this->nombreTablaAsociada	= "adicionales";
		$this->formulario		= "";
		$this->listado			= "lists/imprimirInspeccionesList";
	...
	function action_index() {
        ...
        $inspeccionesSolicitadas = Array();
        //$adicionales = Array();
        if($expediente != null && $expediente->pk() != null) {
            $inspeccionesSolicitadas = ORM::factory($this->nombreTabla)->getInspeccionesSolicitadas($idExpediente);
        } else {
            $idExpediente = null;
            $expediente = null;
        }
        ...
        $this->template->body = View::factory( $this->listado )
            ...
            ->bind( "inspeccionesSolicitadas", $inspeccionesSolicitadas )
    ...
    function action_new() {
        ...
        if($expediente == null || $expediente->pk() == null) {
            ...
            $nombreArchivo = "inspecciones_" . ($expediente->definitivo == 1 ? "expediente" : "temporal")  . "_" . date("Y-m-d-H-i-s");
            $titulo = "inspecciones";
            ...
            if( $inspeccionesSolicitadas != null ) {
                foreach( $inspeccionesSolicitadas as $value ) {
